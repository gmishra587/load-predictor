...
import pandas as pd
from prophet import Prophet
from power.models import RegionHourlyLoad, StateDailyLoad, StateLoad5Min, WeatherHourly
from power.utils.metadata import add_calendar_features
from django.db.models import Avg
...
# ---------- Merge Weather Data ----------
def merge_weather(df: pd.DataFrame, states: list[str]):
    weather = pd.DataFrame(
        WeatherHourly.objects
        .filter(state__in=states)
        .values("datetime", "temperature_c")
